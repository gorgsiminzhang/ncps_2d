...
from ..wirings.wirings import FullyConnected
from ..wirings.wirings import Wiring as BaseWiring
...
class LTC(tf.keras.layers.RNN):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(units, BaseWiring):
            wiring = units
        elif isinstance(units, int):
            wiring = FullyConnected(units)
        else:
            raise TypeError(f"LTC expects an int or Wiring object, got {type(units)}")
