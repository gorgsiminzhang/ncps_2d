...
class AutoNCP(NCP):
    ...
    def __init__(self, height, width, connectivity='4', self_connections=False, seed=None):
        """
        Grid wiring for 2D neuron layout, with optional mismatch to sensory input shape.
        :param height: height of the neuron grid
        :param width: width of the neuron grid
        :param connectivity: '4' (up/down/left/right) or '8' (including diagonals)
        :param self_connections: whether to allow self-loops
        :param seed: random seed
        """
        super().__init__(units=height * width)
        self.height = height
        self.width = width
        self.connectivity = connectivity
        self.self_connections = self_connections
        self.seed = seed
        self.output_dim = 0  # optional

    def _get_connectivity_offsets(self):
    ...
    def build(self, input_shape):
        """
        Builds connections from sensory grid to neuron grid.
        :param input_shape: (H, W) shape of sensory input
        """
        if not (isinstance(input_shape, tuple) and len(input_shape) == 2):
            raise ValueError("input_shape must be a 2D tuple (H, W)")

        input_h, input_w = input_shape
        self.input_dim = input_h * input_w
        self.sensory_adjacency_matrix = np.zeros((self.input_dim, self.units), dtype=np.int32)
        self.adjacency_matrix = np.zeros((self.units, self.units), dtype=np.int32)

        # Project sensory neurons onto neuron grid (resized mapping)
        for si in range(input_h):
            for sj in range(input_w):
                sensory_idx = si * input_w + sj

                # Downscale coordinates to neuron grid
                ni = int(si * self.height / input_h)
                nj = int(sj * self.width / input_w)
                neuron_idx = ni * self.width + nj

                self.add_sensory_synapse(sensory_idx, neuron_idx, polarity=1)
                self.sensory_adjacency_matrix[sensory_idx, neuron_idx] = 1

        # Neuron-to-neuron connections
        for i in range(self.height):
            for j in range(self.width):
                idx = i * self.width + j
                for di, dj in self._get_connectivity_offsets():
                    ni, nj = i + di, j + dj
                    if 0 <= ni < self.height and 0 <= nj < self.width:
                        neighbor_idx = ni * self.width + nj
                        if self.self_connections or idx != neighbor_idx:
                            self.add_synapse(idx, neighbor_idx, polarity=1)
                            self.adjacency_matrix[idx, neighbor_idx] = 1
...
class GridWiring(Wiring):
    def __init__(self, neuron_height, neuron_width, connectivity='8', self_connections=False, seed=None):
        self.neuron_height = neuron_height
        self.neuron_width = neuron_width
        self.connectivity = connectivity  # '4' or '8'
        self.self_connections = self_connections
        self.seed = seed
        self.units = neuron_height * neuron_width
        self.output_dim = self.units  # Treat all as output/inter for visualization
    ...
    def build(self, input_shape):
        ...
        Accepts:
        - (T, H, W) from LTC
        - (H, W) if built manually
        Keeps spatial abstraction aligned with neuron grid.
        """
        
        if isinstance(input_shape, tuple):
            if len(input_shape) == 3:
                _, sensory_h, sensory_w = input_shape  # from LTC.build (T, H, W)
            elif len(input_shape) == 2:
                sensory_h, sensory_w = input_shape      # manual build (H, W)
            else:
                raise ValueError(f"Unsupported input shape: {input_shape}")
        else:
            raise ValueError(f"GridWiring.build() expects tuple, got: {input_shape}")

        self.set_input_dim(sensory_h * sensory_w)
        self.set_output_dim(self.units)  # ✅ set motor_size = total neuron count

        rng = np.random.default_rng(self.seed)

        # Sensory → Neuron projection (spatial downscaling)
        # Sensory → Neuron full connectivity
        for s_idx in range(sensory_h * sensory_w):
            for n_idx in range(self.units):
                polarity = rng.choice([-1, 1])
                self.add_sensory_synapse(s_idx, n_idx, polarity)


        # Neuron ↔ Neuron local topology
        for i in range(self.neuron_height):
            for j in range(self.neuron_width):
                src = i * self.neuron_width + j
                for di in [-1, 0, 1]:
                    for dj in [-1, 0, 1]:
                        if self.connectivity == '4':
                            if abs(di) + abs(dj) == 1:
                                pass  # 4-connected neighbors
                            elif di == 0 and dj == 0 and self.self_connections:
                                pass  # allow self-loop explicitly
                            else:
                                continue
                        elif self.connectivity == '8':
                            if not self.self_connections and di == 0 and dj == 0:
                                continue
                        else:
                            raise ValueError(f"Unsupported connectivity: {self.connectivity}")
                        ni, nj = i + di, j + dj
                        if 0 <= ni < self.neuron_height and 0 <= nj < self.neuron_width:
                            dst = ni * self.neuron_width + nj
                            polarity = rng.choice([-1, 1])
                            self.add_synapse(src, dst, polarity)

    

    def get_type_of_neuron(self, neuron_id):
        return "inter"  # All shown as inter for plotting simplicity
