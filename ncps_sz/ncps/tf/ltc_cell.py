...
class LTCCell(tf.keras.layers.AbstractRNNCell):
    ...
    def build(self, input_shape):
        if isinstance(input_shape, (tuple, tf.TensorShape)):
            if len(input_shape) == 3:
                # Assume (T, H, W) → spatial input
                _, h, w = input_shape
                self._wiring.build((h, w))
            else:
                # Use last dim as input_dim (fallback)
                input_dim = input_shape[-1]
                self._wiring.build(input_dim)
        elif isinstance(input_shape, list) and isinstance(input_shape[0], (tuple, tf.TensorShape)):
            # Keras sometimes passes a list of shapes
            shape = input_shape[0]
            if len(shape) == 3:
                _, h, w = shape
                self._wiring.build((h, w))
            ...
            raise ValueError(f"Unsupported input_shape format: {input_shape}")
        '''
    ...
    def _map_inputs(self, inputs):
        # If spatial input, reshape (B, H, W) → (B, D)
        if tf.rank(inputs) == 3:
            inputs = tf.reshape(inputs, [-1, self.sensory_size])  # ✅ use sensory_size
